...
- [ ] 各施設に入った後、welcome_messageの表示を行う_on_enter関数処理は不要

### 優先度:中

- [ ] パーティの共有インベントリは廃止。アイテム購入時は宿屋の倉庫へ搬入される
- [ ] 宿屋-冒険の準備-アイテム整理で、各キャラクターのアイテム整理を実行出来るようにする
  アイテム整理とは:
    - 宿屋の倉庫とキャラクタ間のアイテムの移動
    - 魔術習得アイテム、祈祷習得アイテムを消費した魔術・祈祷の習得
- [ ] 宿屋-冒険の準備-魔術/祈祷スロットで、各キャラクターの魔術/祈祷スロットへの登録を実装する
  - NOTICE:詳細実装前に、./docs/spell-specs.mdに仕様をまとめること
- [ ] 宿屋-冒険の準備-パーティ装備確認で、各キャラクターのアイテムの装備UIを実装する
