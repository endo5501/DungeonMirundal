...
**Day 21-22: 既存システム分析・統合設計**
- [x] **既存実装調査完了**: ダンジョン・戦闘システム実装状況確認
- [ ] 地上部（Phase 4施設）⇔ダンジョン入場システム統合
- [ ] `OverworldManager`からダンジョン進入フローの実装
- [ ] `DungeonManager`と`CombatManager`の連携強化
- [ ] エンカウンター発生時の自動戦闘開始処理

**Day 23-24: 戦闘統合・フロー改善**
- [ ] 戦闘終了後のダンジョン復帰処理
- [ ] パーティステータス（HP/MP/装備）の戦闘反映確認
- [ ] 死亡・蘇生処理とダンジョン進行の連携
- [ ] Phase 4装備・魔法システムの戦闘内活用確認

**Day 25-26: 欠落機能実装**
- [ ] **トラップ発動システム**: 既存trap定義の実際の発動処理
- [ ] **宝箱開封システム**: 宝箱インタラクション・開封UI
- [ ] **ボス戦特別処理**: ボス撃破時の特別報酬・演出
- [ ] ダンジョン内回復・休息システム（制限付き）
...
- [x] **戦闘システム**: 既に完全実装済み ✅  
- [ ] **Phase 4システムとの完全統合**: 地上部⇔ダンジョン⇔戦闘連携
- [ ] **欠落機能の補完**: トラップ・宝箱・ボス戦処理
- [ ] **セーブ・ロード完全対応**: 統合システム対応
- [ ] **品質保証**: 統合テスト・バランス調整・エラーハンドリング

**Phase 5完了により、既存の優秀なダンジョン・戦闘システムとPhase 4の新施設システムが完全統合され、高品質なWizardry風ダンジョン探索RPGが完成します。**
